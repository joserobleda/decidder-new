(function(b,d){var c=$("#canvas"),a=$("form#question");$.fn.bindCustomEvents=function(){$(this).find(".async").click(function(){var e=$(this).attr("href"),g=$(this).data("target"),f=$(this).data("hide");
$.get(e,function(h){$(g).html(h).show();$(f).hide();});return false;});$(this).find(".popup").click(function(l){var h=$(this).attr("href"),g=$(this).data("popup-name")||false,i=$(this).data("popup-width")||500,f=$(this).data("popup-height")||280,k=(screen.width/2)-(i/2),j=(screen.height/2)-(f/2);
...
function j(k){i.html(k);}$.getJSON(f,function(k){if(k[0]&&k[0][g]!==d){j(k[0][g]);}});});$(".reveal").click(function(){var l=$(this),h=l.data("reveal-target")||null,m=l.data("reveal-src")||null;
function p(){c.css({height:"100%",overflow:"hidden"});}function i(){c.css({height:"inherit",overflow:"auto"});}function k(s){var r=$(s),t=l.data();t.open=function(){r.find("input,textarea,select").not(":disabled").filter(":first").focus();
...
k(r);}}});}});};if(a.length){require(["widget/formEditable"],function(){a.editable(function(e){$(e).bind("edit",function(){$("input.taglist").tagsInput({defaultText:"add predefined response",ready:function(){$(this).attr("data-form-editing",true);
